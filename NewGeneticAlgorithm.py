...
tasks = np.genfromtxt('vehicle_data.csv', delimiter=',', names=True, dtype=None, encoding='UTF-8')
...
def Price_Probability(ρ, MIN, MAX):
    ...
    :return:
    """
    d_min = np.max(ρ)    # 当前数据最大值
    d_max = np.min(ρ)    # 当前数据最小值
    return MIN +(MAX - MIN)/(d_max - d_min) * (ρ - d_min)
...
def Custom_Repair0(x):
    ...
    x1 = np.where(variation[:, :high_price] > ρ_map[:, :high_price], 1, 0)
