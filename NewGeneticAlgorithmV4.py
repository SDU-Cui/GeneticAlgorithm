...
'''
这是遗传算法的版本V4 因为V3repair3中三相不平衡修复存在死循环,修复太困难，
所以将三相不平衡作为软约束放在适应度值里 α * np.sum(result_phase)
用的是csv的数据 试了200、300、400辆车的数据
'''
...
def Custom_Repair3(x):
    ...
    for j in range(len(x[0])):
        ...
        while (used_power[j] - restriction_in_power[j] > 0):
            ...
            x[max_row][j] = 0
            power_loss = power_loss[j:upper_bound]
            if len(power_loss) > 0:
